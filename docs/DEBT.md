...
| # | Item | Severidade | Rationale | Caminho para resolver |
|---|---|---|---|---|
| CK-1 | **Observabilidade do checkup (Sentry)** | ~~🔴 Alta~~ ✅ **Parcial** | Sentry integrado no checkup (`@sentry/nextjs`, instrumentation + withSentryConfig), endurecido p/ LGPD: `sendDefaultPii:false`, **sem session replay** (gravaria as respostas), `beforeSend` raspa body/cookies/headers/query/sid, sem variáveis locais nos frames. Server-side reporta em runtime via `CHECKUP_SENTRY_DSN` (DSN próprio, isolado do clínico) — cobre a classe do incidente (o 500 do `/api/pdf` era server-side). **Pendente (🟡):** (a) provisionar o DSN no `.env` do EC2 (criar projeto Sentry do checkup); (b) **client-side** precisa de `NEXT_PUBLIC_SENTRY_DSN` no BUILD — hoje o build do EC2 não passa, então erros de JS do browser ainda não sobem (plumbing de build-arg no Dockerfile/docker-bake). | Setar `CHECKUP_SENTRY_DSN` no EC2; adicionar build-arg p/ o DSN público no Dockerfile/bake. |
