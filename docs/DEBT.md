...
| # | Item | Severidade | Rationale | Caminho para resolver |
|---|---|---|---|---|
| CK-4 | **`test_results` (consentido) + `report_emails` (e-mail PDF)** | ~~🟡 Média~~ ✅ **Resolvido (código) / 🟡 dep. ops** | **Parte A FEITA+verificada** (commit 00b2e37): consent checkbox → `/api/result` grava `test_results` só com `consented:true`; bug bigserial→integer corrigido. **Parte B FEITA** (commit 209fbab): `/api/email-report` gera PDF + grava `report_emails` (hash bcrypt, e-mail bruto nunca guardado). **Provider de e-mail trocado de AWS SES → Resend (ADR-061, 2026-06-16)** — remove a dependência de SES production-access (que travava o envio a e-mail não-verificado) e alinha ao provider já usado no lado clínico; dado-store segue no RDS sa-east-1, muda só o transporte. **Código COMPLETO (2026-06-20):** UI do campo de e-mail na tela `/resultado` já implementada (input+handler+estados+nota LGPD+a11y); **bug corrigido** — `/api/email-report` aceitava só `phq9/gad7/asrs18` → as 5 escalas expandidas (AUDIT/MDQ/Fagerström/MSI-BPD/ASSIST, live ADR-048/049) baixavam PDF mas davam **400 no envio**; enum espelhado ao `/api/result` (8 escalas). + evento de funil `email_report_sent` (cockpit ADR-050, sem PII) + smoke do `/api/email-report` (fail-closed 502 sem key / 400 input inválido; scale=audit trava o enum). tsc+build+smoke verdes. **Pendente = só OPS:** `RESEND_API_KEY` no SSM/compose do checkup + domínio verificado no Resend (sem isso o envio é fail-closed 502 — não quebra o resto). | OPS: setar RESEND_API_KEY no checkup + verificar domínio no Resend. |
